...
% 略快（0 11 2 4 1 15）
Ka = 0* eye(2); % 加速度控制增益     #### 预期队形变化
K1 = 11 * eye(2);  % 位置控制增益    #### 期望位置跟踪
K2 = 2 * eye(2);  % 邻居位置控制增益 #### 队形张开固定作用
Kv1 = 4 * eye(2); % 速度控制增益     #### 期望速度跟踪
Kv2 = 1 * eye(2); % 邻居速度控制增益 #### 相对速度控制
Ki = 15 * eye(2);  % 积分控制增益     #### 稳态误差减少
