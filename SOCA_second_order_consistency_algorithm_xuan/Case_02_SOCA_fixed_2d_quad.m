% 五个智能体在圆周上形成正边形的固定编队
% 注意修改编队形式，同时修改desired_position and desired_velocity; 以及智能体数量与编队显示连线
...
% ######计算期望位置#######
desired_positions = [r * cos(theta(1)),r * cos(theta(2)),r * cos(theta(3)),r * cos(theta(4)),0; r * sin(theta(1)),r * sin(theta(2)),r * sin(theta(3)),r * sin(theta(4)),0];% 根据需要提供编队形式
...
for t = 1:num_steps
    ...
    for i = 1:n-2
        next_i = mod(i, n) + 1;
        plot([positions(1, i), positions(1, next_i)], [positions(2, i), positions(2, next_i)], '--', 'Color', [0,0,0], 'LineWidth', 2);
    end
    plot([positions(1, n-1), positions(1, 1)], [positions(2, n-1), positions(2, 1)], '--', 'Color', [0,0,0], 'LineWidth', 2);
    % 绘制起始点
    for i = 1:n
...
% 期望位置：
% 
% 计算期望位置 desired_positions，使智能体在圆周上均匀分布，形成正五边形。
% 主仿真循环：
% 
