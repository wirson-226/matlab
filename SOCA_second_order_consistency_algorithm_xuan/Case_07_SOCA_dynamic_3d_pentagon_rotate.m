...
velocities_des_history = zeros(3, n, num_steps);
...
for t = 1:num_steps
    ...
    z = z_init + v_z * dt * (t-1);
    % ######计算期望位置#######
    desired_positions = [r * cos(theta); r * sin(theta); ones(1, n) * z];% 根据需要提供编队形式

    % ######计算期望速度#######
    desired_velocities = w * [r * -sin(theta); r * cos(theta); v_z * ones(1, n)];% 根据需要提供编队形式: 逆时针绕圆
    ...
    velocities_des_history(:, :, t) = desired_velocities;
...
for dim = 1:3
    ...
    for i = 1:n
        plot(time_vector, squeeze(velocities_des_history(dim, i, :)), '--', 'Color', colors(i,:), 'LineWidth', 1.5);
    end
        title(['智能体速度（维度方向按序x,y ' num2str(dim) '）随时间变化']);
    xlabel('时间');
    ylabel(['速度（维度方向按序x,y' num2str(dim) '）']);
    % legend(arrayfun(@(x) ['智能体' num2str(x)], 1:n, 'UniformOutput', false));
    legend([arrayfun(@(x) ['智能体' num2str(x)], 1:n, 'UniformOutput', false), '期望速度'], 'Location', 'Best');
end
