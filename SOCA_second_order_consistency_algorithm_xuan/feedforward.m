...
% 控制增益矩阵
K1 = kron(eye(2), [0.5, 0.5]);  % 使用 Kronecker 积计算增益矩阵
K2 = 3 * eye(2);  % 邻居位置控制增益
Kv = 1 * eye(2);  % 速度控制增益
Kf = 0 * eye(2);  % 前馈控制增益
...
for t = 1:num_steps
    ...
    for i = 1:n
        ...
        % 前馈控制力
        feedforward_control = Kf * desired_velocities(:, i);
        ...
        % 总控制力
        control_force = u1 + u2 + feedforward_control;
