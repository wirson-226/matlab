% 五个二阶系统智能体  在圆周上形成  正五边形的  匀速平移  时变编队
% 假设全连接，时间离散化
% 取消积分项，保证一致性控制协议完整性
...
% 略快 （0 11 2 4 1 15）
Ka = 0* eye(2); % 加速度控制增益     0   ### 预期队形变化
K1 = 10 * eye(2);  % 位置控制增益   10   #### 期望位置跟踪  初始跟踪作用大
K2 = 3 * eye(2);  % 邻居位置控制增益 2   #### 队形张开固定作用 3  2  2
Kv1 = 8 * eye(2); % 速度控制增益     6   #### 期望速度跟踪    8  4  7
Kv2 = 1 * eye(2); % 邻居速度控制增益 1   #### 相对速度控制
% Ki = 15 * eye(2);  % 积分控制增益   10   #### 稳态误差减少 开始偏移多 15
...
% ######计算期望位置#######
desired_positions_init = [r * cos(theta); r * sin(theta)];% 根据需要提供编队形式

% ######计算期望速度#######
desired_velocities_init = 0.2 * ones(2, n);% 根据需要提供编队形式
