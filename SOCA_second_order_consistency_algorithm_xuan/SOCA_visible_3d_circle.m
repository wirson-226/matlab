...
for i = 2:num_agents
    ...
    positions(i, 1:2) = center_position(1:2) + [radius * cos(angles(i)), radius * sin(angles(i))];
    velocities(i, 1:2) = [-radius * sin(angles(i)) * angular_velocity, radius * cos(angles(i)) * angular_velocity];
    % 设置垂直方向位置和速度
    positions(i, 3) = rand() * 10; % 初始垂直位置
    velocities(i, 3) = rand() * 1 + vertical_velocity; % 初始垂直速度
