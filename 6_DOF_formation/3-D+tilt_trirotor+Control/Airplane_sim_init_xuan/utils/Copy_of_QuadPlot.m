classdef Copy_of_QuadPlot < handle
    ...
    properties (SetAccess = public)
        ...
        positions;      % 位置
    attitudes;          % 姿态
        tilt_angle = [0, 0];  % tilt angle --- todo state(14:15)
    ...
    properties (SetAccess = private)
        h_3d
        h_ttr;  % planeplot_ttr_test plot handle
    ...
    methods
        ...
        %     rotor_a_points_rotated = rotor_a_points_rotated + position;% a
        %     rotor_b_points_rotated = rotor_b_points_rotated + position;% b
        % 
        % 
        ...
        %         color_B = [0.1, 0.8, 0.1]; % Green
        % 
        %         % Plot the triangle (face) with the selected color
        %         fill3([p1(1), p2(1), p3(1)], ...
        %               [p1(2), p2(2), p3(2)], ...
        %               [p1(3), p2(3), p3(3)], color_B, 'FaceAlpha', 0.7);
        %     end
        % 
        ...
        %         color_else = [0.1, 0.2, 0.9]; % Blue
        % 
        %         % Plot the triangle (face) with the selected color
        %         fill3([p1(1), p2(1), p3(1)], ...
        %               [p1(2), p2(2), p3(2)], ...
        %               [p1(3), p2(3), p3(3)], color_else, 'FaceAlpha', 0.7);
        %     end
        % 
        % 
        %     %% 添加原点显示
        ...
        % Constructor
        function Q = Copy_of_QuadPlot(qn, state, wingspan, height, color, max_iter, h_3d)
            Q.qn = qn;
            Q.state = state;
            Q.wingspan = wingspan;
            Q.color = color;
            Q.height = height;
            Q.rot = QuatToRot(Q.state(7:10)); % 欧拉角旋转矩阵
            % Q.positions = Q.state(1:3)';       % 位置            
            % Q.attitudes = RotToRPY_ZXY(Q.rot)';% 旋转矩阵到roll pitch yaw 姿态角
            Q.positions = [1,1,1];       % 位置
            Q.attitudes = [0,0,0];% 旋转矩阵到roll pitch yaw 姿态角
            % Q.h_ttr = planeplot_ttr_test(Q.state(1:3), Q.rot, Q.tilt_angle);
        ...
        % Update quad plot
        function UpdateCopy_of_QuadPlot(Q, state, des_state, time)
            Q.UpdateQuadState(state, time);
            Q.UpdateDesiredQuadState(des_state);
            Q.UpdateQuadHist();
            % planeplot_ttr_test(Q.positions, Q.attitudes, Q.tilt_angle);
