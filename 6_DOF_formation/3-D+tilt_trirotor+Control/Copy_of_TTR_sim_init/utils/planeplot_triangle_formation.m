function planeplot_triangle_formation()
    % 动画总时间和时间步
    % 添加VTOL mode判断 待修改匹配倾转角度
    total_time = 6; % 总时间 (秒)
    dt = 0.1; % 时间步
    ...
    while sim_time < total_time
        ...
        % 更新 UAV 位置
        for i = 1:num_agents
            [position(i, :), attitude(i, :)] = update_state(sim_time, total_time, position(i, :), attitude(i, :), dt, mode, is_vtol_mode);
        end
...
function [new_position, new_attitude] = update_state(sim_time, total_time, position, attitude, dt, mode, is_vtol_mode)
    speed = 0; 
    ascent_speed = 0;  
