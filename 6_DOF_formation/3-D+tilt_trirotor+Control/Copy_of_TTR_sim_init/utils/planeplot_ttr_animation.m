function planeplot_ttr_animation()
    ...
    % 初始化状态
    position = [3, 3, 3]; % 初始位置 XYZ
    attitude = [0, 0, 0]; % 初始姿态 [roll, pitch, yaw] rad
    tilt_angle = [10, 10];  % 初始电机倾转角度 [a, b] degrees
    ...
    ylabel('Y (m)');
    zlabel('Z (m)');
    title('TTR-DW Animation'); % Tilt Tri-Rotor Delta-Wing VTOL 倾转三旋翼飞翼布局垂直起降飞行器
    view(3);
...
function [new_position, new_attitude] = update_state(sim_time, total_time, position, attitude, dt)
    ...
    if sim_time < total_time / 6
        position(1) = position(1) + 10 * dt;  % x方向 (东向）平移 右
    elseif sim_time < 2 * total_time / 6
        position(2) = position(2) + 10 * dt;  % Y方向（北向）平移 前
    elseif sim_time < 3 * total_time / 6
        position(3) = position(3) + 10 * dt;  % Z方向（高度）增加 上
    elseif sim_time < 4 * total_time / 6
        attitude(3) = attitude(3) + 0.5 * dt;  % 航向角变化（Yaw）  psi 
    elseif sim_time < 5 * total_time / 6
        attitude(2) = attitude(2) + 0.5 * dt;  % 俯仰角变化（Pitch）theta 
    else
        attitude(1) = attitude(1) + 0.5 * dt;  % 滚转角变化（Roll） phi 
    end
