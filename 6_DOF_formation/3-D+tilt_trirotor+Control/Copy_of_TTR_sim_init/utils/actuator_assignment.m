function [actuator] = actuator_assignment(force, moment, state, params, mode)
    %   反向执行器分配，基于输入的  机体系期望合力和力矩  解算执行器输出 rotor abc + elevon rl + tilt angle ab
    %   mode 1 = copter ; mode 2 = cruise;
    %   坐标系 东北天 NEU 对应 右前上 RFU XYZ- pitch roll yaw
    ...
    p = state.omega(1); % roll  phi
    q = state.omega(2); % pitch theta
    r = state.omega(3); % yaw   psi
