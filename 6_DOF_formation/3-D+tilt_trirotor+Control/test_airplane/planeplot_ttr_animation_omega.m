function planeplot_ttr_animation_omega()
    % 动画总时间和时间步
    total_time = 6; % 总时间 (秒)
    dt = 0.1; % 时间步 (秒)
    ...
    for step = 1:num_steps
        ...
        elseif sim_time < 4 * total_time / 6
            attitude(3) = attitude(3) + 0.1 * dt;  % 航向角变化（Yaw）
        elseif sim_time < 5 * total_time / 6
            attitude(2) = attitude(2) + 0.1 * dt;  % 俯仰角变化（Pitch）
        else
            attitude(1) = attitude(1) + 0.1 * dt;  % 滚转角变化（Roll）
        end
        ...
        omega(1) = 50 * sin(2 * pi * sim_time / total_time);  % Rotor A 转
        omega(2) = 50 * cos(2 * pi * sim_time / total_time);  % Rotor B 转
        ...
        % 调用绘图函数，不再创建新窗口
        planeplot_ttr_test(position, attitude, tilt_angle,omega);  % 更新同一窗口
