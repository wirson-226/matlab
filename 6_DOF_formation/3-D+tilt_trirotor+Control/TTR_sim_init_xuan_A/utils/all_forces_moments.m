function [force, moment] = all_forces_moments(state, wind_speed, actuator, params)
    ...
    u_r = state(4) - wind_speed(1);
    v_r = state(5) - wind_speed(2);
    w_r = state(6) - wind_speed(3);
    ...
    if Va == 0
        beta = 0;  % If airspeed is zero, set beta to zero
    else
        beta = asin(v_r / Va);
    end
    ...
    fx = F_lift * cos(alpha) - F_drag * sin(alpha) + thrust_prop_a_x + thrust_prop_b_x;
    fz = F_lift * sin(alpha) + F_drag * cos(alpha) - thrust_prop_a_z - thrust_prop_b_z;

    % Compute lateral forces in body frame
    fy = rVS_2 * (C_Y_0 + C_Y_beta * beta + C_Y_p * MAV.b / (2 * Va) * p + C_Y_r * MAV.b / (2 * Va) * r + MAV.C_Y_delta_a * aileron);
