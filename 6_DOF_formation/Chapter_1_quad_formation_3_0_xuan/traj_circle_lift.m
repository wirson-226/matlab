function desired_state = traj_circle_lift(t, agent_index)
    t_max = 20;
    t = max(0, min(t, t_max));
    t_norm = t / t_max;
    ...
    vel_base = radius * angular_speed * [-sin(angle); cos(angle); 1/t_max]; % Velocity with constant ascent rate
    acc_base = radius * angular_speed^2 * [-cos(angle); -sin(angle); 0]; % Acceleration only affects horizontal plane
