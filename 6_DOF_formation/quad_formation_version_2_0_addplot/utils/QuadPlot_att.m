classdef QuadPlot < handle
    ...
    properties (SetAccess = public)
        ...
        att_hist;         % attitudes record history
        att_des_hist;     % desired attitudes record history
    ...
    methods
        ...
        function Q = QuadPlot(qn, state, wingspan, height, color, max_iter, h_3d)
            ...
            Q.att = RotToRPY_ZXY(Q.rot); % added attitudes record
            ...
            Q.att_hist = zeros(3, max_iter);
            Q.att_des_hist = zeros(3, max_iter);
        ...
        function UpdateQuadState(Q, state, time)
            ...
            Q.att = RotToRPY_ZXY(Q.rot');    % added
        end

        % Update desired quad state
        function UpdateDesiredQuadState(Q, des_state)
            Q.des_state = des_state;
            % Q.rot_des = QuatToRot()'; % Q.rot needs to be body-to-world
            % Q.att_des = RotToRPY_ZXY(Q.rot_des');    % added
        end
        ...
        function UpdateQuadHist(Q)
            ...
            Q.rot = QuatToRot(Q.state(7:10))'; % Q.rot needs to be body-to-world
            Q.att = RotToRPY_ZXY(Q.rot');    % added
            Q.att_hist(:,Q.k) = Q.att(1:3); % 添加姿态角状态记录
            % Q.att_des_hist(:,Q.k) = Q.att_des(1:3); % 添加期望姿态角状态记录
        end
        ...
        function TruncateHist(Q)
            ...
            Q.att_hist = Q.att_hist(:, 1:Q.k); % added
            Q.att_des_hist = Q.att_hist(:, 1:Q.k); % added
        end
