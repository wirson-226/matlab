...
function plot_trajectory(pos_hist, center_hist)
    ...
    for i = 1:N
        ...
        scatter(ax, pos_hist(i,1,1), pos_hist(i,2,1), 60, 'o', 'filled', ...
            'MarkerFaceColor', color, 'MarkerEdgeColor', color);   % 起点
        scatter(ax, pos_hist(i,1,end), pos_hist(i,2,end), 120, '^', ...
            'MarkerFaceColor', color, 'MarkerEdgeColor', color);  % 终点
    end

    % 绘制中心轨迹
    if nargin > 1 && ~isempty(center_hist)
        plot(center_hist(:,1), center_hist(:,2), 'k--', 'LineWidth', 1.5);
        scatter(center_hist(1,1), center_hist(1,2), 100, 'p', ...
            'MarkerEdgeColor','k', 'MarkerFaceColor','none');  % 起点
        scatter(center_hist(end,1), center_hist(end,2), 150, 'p', ...
            'MarkerEdgeColor','k', 'MarkerFaceColor','y');     % 终点
    end

    xlabel(ax, 'X [m]'); ylabel(ax, 'Y [m]');
    title(ax, 'Agent Trajectories and Target Center Path');
    axis(ax, 'equal'); grid(ax, 'on');

    % 简洁图例（仅一部分元素）
    plot(nan, nan, 'k--', 'LineWidth', 1.5, 'DisplayName', 'Center Path');
    plot(nan, nan, 'p', 'MarkerFaceColor', 'y', 'MarkerEdgeColor', 'k', 'DisplayName', 'Center End');
    legend(ax, 'show', 'Location', 'northeastoutside');
end
