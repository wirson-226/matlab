...
%     for i = 1:N
%         color = colororder(i,:);
%         plot(squeeze(pos_hist(i,1,:)), squeeze(pos_hist(i,2,:)), '-', 'LineWidth', 1, 'Color', color,'DisplayName', sprintf('Agent %d', i));  % 轨迹线
%         scatter(pos_hist(i,1,1), pos_hist(i,2,1), 60, 'o', 'filled', 'MarkerFaceColor', color, 'MarkerEdgeColor', color, 'DisplayName', sprintf('Agent %d Start', i));   % 起点
%         scatter(pos_hist(i,1,end), pos_hist(i,2,end), 120, '^', 'MarkerFaceColor', color, 'MarkerEdgeColor', color, 'DisplayName', sprintf('Agent %d End', i));  % 终点
%     end
