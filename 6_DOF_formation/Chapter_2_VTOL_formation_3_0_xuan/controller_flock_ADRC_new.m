...
function [F, M, ATT_des] = controller_flock_ADRC_new(t, state, des_state, params, global_state, global_des_state, agent_index)
% CONTROLLER_FLOCK_ADRC Flocking controller for the quadrotor using ADRC
% Implements flocking control using desired acceleration as input to the attitude controller with ADRC
...
% ADRC parameters
persistent z_pos z_att

if isempty(z_pos)
    z_pos = zeros(3, 3);  % [z_x; z_y; z_z] for position ESO states
    z_att = zeros(3, 3);  % [z_phi; z_theta; z_psi] for attitude ESO states
end
...
z_pos(:, 1) = ESO_NL_new(z_pos(:, 1), state_i.pos(1), u_1, params, 'alt');
z_pos(:, 2) = ESO_NL_new(z_pos(:, 2), state_i.pos(2), u_1, params, 'alt');
z_pos(:, 3) = ESO_NL_new(z_pos(:, 3), state_i.pos(3), u_1, params, 'alt');
...
z_att(:, 1) = ESO_NL_new(z_att(:, 1), phi, 0, params, 'roll');
z_att(:, 2) = ESO_NL_new(z_att(:, 2), theta, 0, params, 'pitch');
z_att(:, 3) = ESO_NL_new(z_att(:, 3), psi, 0, params, 'yaw');
...
function z = ESO_NL_new(z, y, u, params, mode)
% ESO_NL: Nonlinear Extended State Observer for ADRC with mode-specific tuning
% z: state vector [z1; z2; z3]
% y: measured output
% u: control input
% params: system parameters
% mode: control mode ('roll', 'pitch', 'yaw', 'alt')

h = 0.01;  % Sampling period
% 
% params.Ix = params.I(1, 1);  % 惯性矩沿 x 轴，值为 0.00025 kg·m²
% params.Iy = params.I(2, 2);  % 惯性矩沿 y 轴，值为 0.000232 kg·m²
% params.Iz = params.I(3, 3);  % 惯性矩沿 z 轴，值为 0.0003738 kg·m²

%%%% 备用  %%%%
params.Ix = 0.00025;      % 惯性矩沿 x 轴
params.Iy = 0.000232;     % 惯性矩沿 y 轴
params.Iz = 0.0003738;    % 惯性矩沿 z 轴



% Mode-specific ESO gains and system parameters
switch mode
    case 'roll'
        beta = [100; 300; 500];  % ESO gains for roll
        b = params.Ix;           % Moment of inertia around x-axis
    case 'pitch'
        beta = [100; 300; 500];  % ESO gains for pitch
        b = params.Iy;           % Moment of inertia around y-axis
    case 'yaw'
        beta = [100; 300; 500];  % ESO gains for yaw
        b = params.Iz;           % Moment of inertia around z-axis
    case 'alt'
        beta = [200; 400; 600];  % ESO gains for altitude
        b = params.mass;         % Mass of the quadrotor
    otherwise
        error('Unknown mode');
end

% Update ESO states
z(1) = z(1) + h * (z(2) - beta(1) * fal(y - z(1), 0.5, h));
z(2) = z(2) + h * (z(3) - beta(2) * fal(y - z(1), 0.5, h) + u / b);
z(3) = z(3) + h * (-beta(3) * fal(y - z(1), 0.5, h));

end

function y = fal(e, a, delta)
% fal: Nonlinear function in ADRC
% e: error input
% a: nonlinearity parameter
% delta: small threshold
