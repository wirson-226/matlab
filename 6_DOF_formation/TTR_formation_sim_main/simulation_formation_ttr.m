...
figHandles = findall(0, 'Type', 'figure', 'Visible', 'on');
for i = 1:length(figHandles)
    fig = figHandles(i);
    figName = get(fig, 'Name');  % 激活 figure
    % 图名及文件名生成
    % 忽略没有 Name 的图像（一般是临时的）
    if isempty(figName) || startsWith(figName, 'Figure')
        continue;
    end
    ...
    figFile = sprintf('%02d_%s.png', i, regexprep(figName, '[^\w]', '_'));  % 去除非法字符

    % 强制刷新 GUI 并保存
    drawnow expose;
    print(fig, fullfile(save_dir, figFile), '-dpng', '-r1000');
end
