function planeplot_ttr_animation_ori()
    % 动画总时间和时间步
    total_time = 6; % 总时间 (秒)
    dt = 0.1; % 初始时间步 (秒)
    ...
    % 初始化状态
    position = [10, 10, 10]; % 初始位置 XYZ
    attitude = [1, 1, 1]; % 初始姿态 [roll, pitch, yaw]
    ...
    ylabel('Y (m)');
    zlabel('Z (m)');
    title('TTR_DW Animation'); % Tilt Tri-Rotor Delta-Wing VTOL 倾转三旋翼飞翼布局垂直起降飞行器
    view(3);
