...
function [gBestScore,gBest,BestCosts]=MSCPSO(N,Max_iteration,lb,ub,dim,fobj)
    ...
    for j=1:4
     ...
     for it=1:MaxIt
	    ...
	    for j = 1:2
		    for i=1:nPop
                w = adaptiveInertia(slave_particle(i,j).Cost,favg,wmax,wmin,fmin,i,MaxIt);
			    % Update Velocity
...
function y = adaptiveInertia(x,favg,wmax,wmin,fmin,it,MaxIt)	
	%%
    if x >= favg
    %stage1
    w = wmax;
