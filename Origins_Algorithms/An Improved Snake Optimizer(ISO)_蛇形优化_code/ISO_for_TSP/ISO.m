...
if Q<Threshold
    for i=1:Nm
        ...
        [~,solm1]=sort(Xnewm1,2);
        [~,solm2]=sort(Xnewm2,2);
 if fobj(solm2(i,:)) < fobj(solm1(i,:))
              Xnewm(i,:)=Xnewm2(i,:);
        else 
            Xnewm(i,:)=Xnewm1(i,:);
        end
[~,solm]=sort(Xnewm,2);
if(fobj(solm(i,:))<fitness(i))
            X(i,:) = Xnewm(i,:);
fitness(i) = fobj(solm(i,:));
end
    end
    for i=1:Nf
        for j=1:1:dim
...
[~,solf1]=sort(Xnewf1,2);
[~,solf2]=sort(Xnewf2,2);
if fobj(solf2(i,:)) < fobj(solf1(i,:))
              Xnewf(i,:)=Xnewf2(i,:);
else 
            Xnewf(i,:)=Xnewf1(i,:);
end   
[~,solbf]=sort(Xbest_f,2);
[~,solf]=sort(Xnewf,2);
if(fobj(solf(i,:))<fobj(solbf))
            X(i,:) = Xnewf(i,:);
            fitness(i) = fobj(solf(i,:));
        end
    end
else 
    if Temp>Thresold2
            ...
            if egg==1  
              ...
              for i=1:N
                ...
                for i=1:a
                 ...
                 [~,solson]=sort(Xnew_son,2);
                    fit_mson=fobj(solson(I1(i),:));
                   if fit_mson< PAIXU(i)
