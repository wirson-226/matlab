...
% define global variables for the problem in NOMA-MEC systems
global p_max; 
global Num_User;
...
function o = F24(power) 
    ...
    H = (C_no < sum(sqrt(power)));
    penalty = muy*sum(H.*((C_no - sum(sqrt(power))).^2));
    H = (p_max < sum(power));
    penalty = penalty + muy*sum(H.*((p_max - sum(power)).^2));
    ...
    % the minus sign since we are maximizing the log sum rate
    % but the MPA algorithm tries to minimize a fitness function
    %%% objective function (maximization --> minimization problem)
    o = -sum(log(rate)) + penalty;     
end