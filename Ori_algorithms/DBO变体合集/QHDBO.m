function [fMin,bestX,  Convergence_curve] = QHDBO(N, maxEvaluations, c, d, dim, fobj)
    ...
    % 开始更新解
    for evalCount =1: maxEvaluations
...
%         end
    if i>(6/30)*N&&i<=(19/30)*N 
         P=0.2+0.6*evalCount / maxEvaluations; % Equation (4)
         if rand()<P
                      Ridus=((ub-lb)./2).*R;
                                     % 参考文献中的Eq. (12)
                     SC=zeros(1,dim-1);
                     thea1=unifrnd(0,2*pi);
                     SC(1)=sin(unifrnd(0,pi));
                     for k4=2:dim-1
                     SC(k4)=sin(thea1);
                     end
                    K3=rand();
                     for j=1:dim
                        if j==1
                        x(i,:)=bestX(j)+Ridus*K3*cos(unifrnd(0,2*pi));
                        else
                        Res=prod(SC(1:j-1));
                        x(i,:)=bestX(j)+Ridus*K3*Res;
                        end
                     end
                  else 
                     x( i, : )=bestXX+((rand(1,dim)).*(pX( i , : )-Xnew1)+(rand(1,dim)).*(pX( i , : )-Xnew2)); 
...
%      x( i, : )=bestXX+((rand(1,dim)).*(pX( i , : )-Xnew1)+(rand(1,dim)).*(pX( i , : )-Xnew2));
        ...
        for j = 20:N  % 方程 (7)
            x(j, :) = bestX + randn(1, dim) .* ((abs((pX(j, :) - rand*bestXX))) + (abs((pX(j, :) - bestX)))) / 2;
        ...
        for i = 1:N 
            ...
            % 初始最优解
             MSv = x(1, :);
             X_new=MSv+tpdf(MSv,evalCount);
             x(i, : ) = Bounds( X_new,lb, ub);
%              MSv=max(X_new,lu(1));x(i,:)=min(X_new,lu(2));
            fit2 = fobj( MSv );
              if(fit2<fMin )
                 fMin= fit2;
                 bestX  =MSv;
